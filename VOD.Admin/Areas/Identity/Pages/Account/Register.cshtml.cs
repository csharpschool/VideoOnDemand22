        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (identityResult.Succeeded) _logger.LogInformation("Added the VODUser Claim to the User.");
                    identityResult = await _userManager.AddClaimAsync(user, new Claim("Admin", "true"));
                    if (identityResult.Succeeded) _logger.LogInformation("Added the Admin Claim to the User.");

                    // Add admin role to the user
                    identityResult = await _userManager.AddToRoleAsync(user, "Admin");
                    if (identityResult.Succeeded) _logger.LogInformation("Added the Admin Role to the User.");
